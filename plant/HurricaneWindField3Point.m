...
% aircraft position
r = ((x(1)-z(1))^2+(x(2)-z(2))^2)^.5;
theta = atan2((x(2)-z(2)),(x(1)-z(1)));
...
noise = 0;%-Vr/2 + (Vr/2+Vr/2)*rand(1,1);
...
Vt = cross([(Vr*.277778+noise)*(x(1)-z(1))/abs((x(1)-z(1))),(Vr*.277778+noise)*(x(2)-z(2))/abs((x(2)-z(2))),0],[0,0,1]);
...
% 	HEB		=	eye(3);
	windb	=	[Vt(1),Vt(2),dz]';					% Body-axis frame
